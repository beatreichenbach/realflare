...
)
{
	...
	for (int batch_id = 0; batch_id < batch_count; batch_id++) {
		...
		for (int batch_prim_id = 0; batch_prim_id < BATCH_PRIMITIVE_COUNT; batch_prim_id++) {
			...
			int vertex_offset = path_id * vertex_count;

			for (int wavelength_id = 0; wavelength_id < max(wavelength_count - 1, 1); wavelength_id++) {
				v1[0] = vertexes[(vertex_offset + quads.x) * wavelength_count + wavelength_id];
				v1[1] = vertexes[(vertex_offset + quads.y) * wavelength_count + wavelength_id];
				v1[2] = vertexes[(vertex_offset + quads.z) * wavelength_count + wavelength_id];
				v1[3] = vertexes[(vertex_offset + quads.w) * wavelength_count + wavelength_id];

				v2[0] = vertexes[(vertex_offset + quads.x) * wavelength_count + (wavelength_id + 1)];
				v2[1] = vertexes[(vertex_offset + quads.y) * wavelength_count + (wavelength_id + 1)];
				v2[2] = vertexes[(vertex_offset + quads.z) * wavelength_count + (wavelength_id + 1)];
				v2[3] = vertexes[(vertex_offset + quads.w) * wavelength_count + (wavelength_id + 1)];
				...
				for (int i = 0; i < wavelength_sub_count; i++) {
					...
					Vertex v[4];
					for (int j = 0; j < 4; j++) {
						pos[j] = convert_int2(mix(v1[j].pos, v2[j].pos, a) * sub_steps);
						v[j].uv = mix(v1[j].uv, v2[j].uv, a);
						v[j].intensity = mix(v1[j].intensity, v2[j].intensity, a);
						v[j].rrel = mix(v1[j].rrel, v2[j].rrel, a);
						v[j].reflectance = mix(v1[j].reflectance, v2[j].reflectance, a);
					}

					float intensity = 0;
					// intensity += debug_grid_quad(p, pos[0], pos[1], pos[2], pos[3]);
					for(char s = 0; s < sub_steps; s++) {
						// offset sample position based on n-rook pattern
						int2 sample_pos = p + (int2) (s, sub_offsets[s]);
						if (intersect_quad(sample_pos, pos[0], pos[1], pos[2], pos[3])) {
							float4 weights = compute_barycentric_quad(sample_pos, pos[0], pos[1], pos[2], pos[3]);
							intensity += fragment_shader(weights, v[0], v[1], v[2], v[3], ghosts, wavelength_id);
							// intensity += 0.1f;
							// intensity += quad_id;
						}
					}
