...
from flare.utils.ciexyz import CIEXYZ
...
class RasterizingTask(OpenCL):
    ...
    @timer
    def run(
        self, rays: Buffer, ghost: ImageArray, flare: Flare, quality: Render.Quality
    ) -> Image:
        ...
        flare_image.args = (rays, ghost, flare, quality)
        ...
        areas.args = (rays, flare, quality)
        bounds = self.update_bounds(bounds_shape)
        bounds.args = (rays, flare, quality)
        screen_transform = self.update_sensor(resolution, flare.lens.sensor_size)
        area_orig = self.update_area_orig(quality.grid_count, quality.grid_length)
        min_area = flare.lens.min_area * area_orig
        ...
        self.kernels['prim_shader'].set_arg(6, np.float32(min_area))
        ...
        vertexes.args = (rays, flare, quality)
        ...
        bin_queues.args = (rays, flare, quality)
