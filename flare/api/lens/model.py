...
class Lens(HashableModel):
    ...
    def get_coatings(
        ...
        for i, surface in enumerate(self.surfaces):
            ...
            # No coating for similar IOR, the aperture or the sensor
            has_coating = (
                (previous_ior > 1) is (current_ior > 1)
                or surface.type == Lens.Surface.SurfaceType.STOP
                or i == len(self.surfaces) - 1
            )
            if has_coating:
                coating = (0, 0)
            else:
